...
const corebosEvents = new EventEmitter();
const coreBOSSessionExpired = 'coreBOSSessionExpiredEvent';
const coreBOSAuthorizationRequired = 'coreBOSAuthorizationRequiredEvent';
 ...
 * Login Operation
 */
async function doLogin(username, accesskey, withpassword) {
	// reqtype = 'POST';
 ...
 */
async function doLoginPortal(username, password, hashmethod, entity) {
	// reqtype = 'GET';
	let login = false;
		...
		.then(function (data) {
			if (hasError(data) === false) {
				...
				if (sessionValidityDetector(data)) {
					corebosEvents.emit(coreBOSSessionExpired);
				}
				if (authorizationValidityDetector(data)) {
					corebosEvents.emit(coreBOSAuthorizationRequired);
				}
				return Promise.reject(new Error('incorrect response: '+lastError()));
			}
		})
 ...
 */
function doQueryWithTotal(query) {
	if (query.indexOf(';') === -1) {
		query += ';';
	}
 ...
 * Upsert Operation
 */
function doUpsert(module, createFields, searchOn, updateFields) {
	// reqtype = 'POST';
 ...
 */
function doMassUpdate(elements) {
	// reqtype = 'POST';
 ...
 */
function doMassUpsert(elements) {
	// reqtype = 'POST';
 ...
 */
function doValidateInformation(record, module, recordInformation) {
	// reqtype = 'POST';
...
module.exports = {
	...
	doMassDelete,
	doInvoke,
	doValidateInformation,
	doGetRelatedRecords,
	doSetRelated,
	corebosEvents,
	authorizationValidityDetector,
